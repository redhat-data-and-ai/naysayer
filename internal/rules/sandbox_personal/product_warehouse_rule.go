...
func (r *ProductWarehouseRule) ValidateLines(filePath string, fileContent string, lineRanges []shared.LineRange) (shared.DecisionType, string) {
	...
	// EXISTING file - delegate to standard warehouse_rule (no changes to warehouse package)
	if !isNewFile {
		logging.Info("[%s] EXISTING file detected, delegating to warehouse_rule", r.Name())
		whRule := warehouse.NewRule(r.client)
		whRule.SetMRContext(r.mrContext)
		decision, reason := whRule.ValidateLines(filePath, fileContent, lineRanges)
		if decision == shared.ManualReview {
			return r.CreateManualReviewResult(reason)
		}
		return r.CreateApprovalResult(reason)
	}
